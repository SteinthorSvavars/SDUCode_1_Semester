...
enum DHT_Status_t DHT_ReadRaw(uint8_t Data[4])
{
	...
	{
		//Request data
		TempSensorWrite(PINB = 0x00);			//DHT_PIN = 0
		...
		TempSensorWrite(DHT_Pin, High);		//DHT_PIN = 1 (Pull-up resistor)
		PinMode(DHT_Pin, Input);			//DHT_PIN = Input

		//Wait for response for 20-40us
		retries = 0;
		while (TempSensorRead(DHT_Pin))
		{
			_delay_us(2);
			retries += 2;
			if (retries > 60)
		...
		for (i = 0 ; i < 5 ; i++)
			for (j = 7 ; j >= 0 ; j--)
			{
				...
				if (__DHT_STATUS == DHT_Ok)
				{
					...
					if (TempSensorRead(DHT_Pin))				//If HIGH
