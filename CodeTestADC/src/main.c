...
{ 
  ...
  DDRC = 0b11110100; // configure pins PC0 to PC3 as inputs
  DDRD = 0xF0;
  DDRB = 0xF0;
  pinMode(17, INPUT); //MotionSensor
  digitalWrite(17, LOW);
  PORTC = 0b00110100; // configure pins PC0 to PC3 to not use pullups for the ADC
  PORTD = 0x00;
  PORTB = 0x00;
  ...
  while(1)
  {
    ...
    if(PINC = 0b00111000 && digitalRead(17) == HIGH){
      PORTD |= _BV(PORTD4);
    }
    else{
      PORTD &=~_BV(PORTD4);
